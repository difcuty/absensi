...
import { Home, BookOpen, TrendingUp, FileText, Settings, X, Camera } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-28">
      {/* CSS FIX: Menghilangkan area hitam & elemen default library */}
      <style>{`
        #reader {
        ...
        #reader video {
          ...
          object-fit: cover !important; /* Membuat kamera full kotak */
          border-radius: 2rem !important;
        }
        #reader__dashboard, #reader__status_span {
          display: none !important; /* Hapus teks instruksi library */
        }
        img[alt="Scanner backdrop"] {
      ...
      <header className="bg-white p-4 shadow-sm sticky top-0 z-50">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <img 
            src={
              !user?.foto_url 
                ? "https://via.placeholder.com/150" // Jika tidak ada foto
                : user.foto_url.startsWith('http') 
                  ? user.foto_url.replace('http://localhost:3000', import.meta.env.VITE_API_URL) 
                  : `${import.meta.env.VITE_API_URL}${user.foto_url}`
            } 
            alt="Profile" 
        ...
        {isScannerOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed bottom-0 left-0 right-0 h-[85vh] bg-[#121212] rounded-t-[3rem] z-[70] overflow-hidden flex flex-col shadow-2xl border-t border-white/10"
            >
                ...
                <div className="relative w-full aspect-square max-w-[320px] overflow-hidden rounded-[2rem] bg-black shadow-2xl">
                  ...
                  <div className="absolute inset-0 pointer-events-none z-20">
                    ...
                    <motion.div 
                      ...
                      className="absolute left-10 right-10 h-0.5 bg-blue-500 shadow-[0_0_15px_#3b82f6]"
                    />
                  </div>
                </div>
...
function MenuCard({ icon, label, to }) {
  return (
    <Link to={to} className="flex flex-col items-center p-5 bg-white rounded-2xl shadow-sm border border-gray-50 hover:shadow-md hover:border-blue-100 transition-all active:scale-95 group text-decoration-none">
      ...
      <span className="text-[10px] font-black text-gray-500 uppercase text-center leading-tight tracking-tighter">
        {label}
      </span>
    </Link>
  );
